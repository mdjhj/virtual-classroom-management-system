...
if(isset($_POST['register_button'])){
    ...
    //role
    $role = strip_tags($_POST['role']); //remove html tag
    $role = str_replace(' ', '', $role); //remove spaces
    $role = ucfirst(strtolower($role)); // uppercase first letter
    $_SESSION['role'] = $role;// stores first name into session variable
    ...
    if(empty($error_array)){
      ...
      $profile_pic = "images/deafultPP.png";
      $query = mysqli_query($con, "INSERT INTO users VALUES ('', '$role', '$fname', '$lname', '$username', '$em', '$password', '$profile_pic', '$date','no','','','')");
